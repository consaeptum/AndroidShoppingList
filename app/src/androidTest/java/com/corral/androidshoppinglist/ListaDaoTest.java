...
public class ListaDaoTest {
    ...
    public void crudTest() throws Exception {
        ...
        f.setFechaFormatYMD("2017-04-7"); f.setId_super(1L);
        assertTrue(fd.insert(f));

        f = fd.read(f.getId()); f.setId_super(2L);
        assertTrue(f != null);

        f.setFechaFormatYMD("2027-04-07"); f.setId_super(3L);
        fd.update(f);

        f = fd.read(f.getId());
        assertTrue(f.getFechaFormatYMD().contains("2027-04-07"));

        assertTrue(fd.delete(f.getId()));

        f.setFechaFormatYMD("2017-04-09"); f.setId_super(1L);
        assertTrue(fd.insert(f));

        f.setFechaFormatYMD("2017-04-10"); f.setId_super(2L);
        assertTrue(fd.insert(f));

        f.setFechaFormatYMD("2017-04-11"); f.setId_super(3L);
        assertTrue(fd.insert(f));
